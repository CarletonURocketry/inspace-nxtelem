...
static enum cons_flag_e {
  CONSUMED = 1,   /* The consumer has used this packet */
  UNCONSUMED = 0, /* The consumer has not used this packet */
};
...
static void *init_consumed(packet_buffer_t *buf, enum cons_flag_e state) {
  for (int i = 0; i < len(buf->consumer_flags); i++) {
 ...
 */
static int was_consumed(packet_buffer_t *buf, cons_id_t id) {
 ...
 */
static void set_consumed(packet_buffer_t *buf, cons_id_t id) {
  return buf->consumer_flags[id] = CONSUMED;
}
...
static int swap_write_buff(packet_queue_t *queue) {
  ...
  int max_consumed = -1;
  int best_buffer = -1;
  for (int i = len(queue->read_queue) - 1; i >= 0; i--) {
...
int queue_init(packet_queue_t *queue) {
  ...
  for (int i = 0; i < len(queue->buffers); i++) {
    ...
    queue->buffers[i].packet_length = QUEUE_MAX_PKT_SIZE;
    if (i == 0) {
...
int queue_write(packet_queue_t *queue, blk_hdr_t *blk_hdr, uint8_t *blk,
  ...
  if (wrbuf(queue)->packet_length + blk_len > len(wrbuf(queue)->blocks)) {
...
cons_id_t queue_add_consumer(packet_queue_t *queue) {
  cons_id_t new_id = atomic_fetch_add(&queue->num_consumers, 1);
  if (new_id <= QUEUE_NUM_CONSUMERS) {
    return new_id;
  }
  return -1;
}
...
packet_buffer_t *queue_get_buffer(packet_queue_t *queue, const cons_id_t id) {
  ...
  for (int i = 0; (i < len(queue->read_queue)) && (output == NULL); i++) {
