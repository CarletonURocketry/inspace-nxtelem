...
static void test_set_flying__flying_loaded(void) { check_set_state(STATE_AIRBORNE); }

/* Test if we can set landed state and load it */
static void test_set_landed__landing_loaded(void) { check_set_state(STATE_LANDED); }

/* Test if we can set the idle state and load it */
static void test_set_idle__idle_loaded(void) { check_set_state(STATE_IDLE); }
